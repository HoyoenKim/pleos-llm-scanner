param(
    [string]$OutputDir = "data/runtime_poc_llm_boundary_demo",
    [int]$Seconds = 250,
    [switch]$SkipBuild
)
...
function Build-And-Install {
    ...
    python scripts/research_runtime_poc_harness.py --build
    if ($LASTEXITCODE -ne 0) { throw "harness build failed" }
    adb uninstall org.codex.pleos.poc 2>$null | Out-Null
    adb install -r data/runtime_poc_harness/poc-harness-signed.apk | Out-Host
...
````powershell
powershell -ExecutionPolicy Bypass -File scripts/record_llm_boundary_demo.ps1
````
