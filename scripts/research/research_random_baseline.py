...
Outputs:
  data/reports/aggregate/random_sample_baseline.md
  data/reports/aggregate/random_sample_baseline.json
"""
...
def decompile_if_needed(apk_path: Path) -> Path:
    """Run scripts/apk/decompile.sh if output dir does not exist. Returns the
    ...
    subprocess.run(
        ["bash", "scripts/apk/decompile.sh", str(apk_path.relative_to(ROOT))],
        cwd=ROOT,
        check=False,
        capture_output=True,
    )
