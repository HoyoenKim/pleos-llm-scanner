...
_작성일: 2026-05-14 / 대상: 교수·평가자 / 기준: `docs/01_report.md` v1.4 supplement + local result artifacts_
...
```java
// data/decompiled/ai.umos.ambientai/.../LlmHandler.java
static {
...
```java
// data/decompiled/ai.umos.appmarket/.../AppMarketActivity.java
...
```xml
<!-- data/decompiled/ai.pleos.playground.account/resources/AndroidManifest.xml -->
<activity
...
```java
// data/decompiled/ai.pleos.playground.account/.../SsoActivity.java
...
```java
// data/decompiled/ai.pleos.sync.syslog/.../SysLogService.java
...
```xml
<!-- data/decompiled/ai.pleos.llm.model.provider/resources/AndroidManifest.xml -->
<provider
...
```java
// data/decompiled/ai.pleos.llm.model.provider/.../PromptsContentProvider.java
...
```xml
<!-- data/decompiled/ai.umos.vehiclecontrol/resources/AndroidManifest.xml -->
<receiver
...
```java
// data/decompiled/ai.umos.vehiclecontrol/.../VehicleBroadcastReceiver.java
...
결론부터 말하면 “봤는데 결과가 없었다”가 아니라, “이번 범위의 static native scan에서는 추가 native-bound 취약점을 만들 근거가 없었다”가 정확하다. Java/Kotlin APK 본문은 `jadx`로 디컴파일했지만, `.so`는 소스 수준 복원이 아니라 radare2/ELF/string surface와 `src/native_analyze.py` 기반의 보수적 정적 분석으로 확인했다. Ghidra Go plugin을 이용한 full decompile은 후속 작업으로 남겨 두었다.
...
- 통합 보고서: `docs/01_report.md`
- 사례 연구: `docs/02_case_studies.md`
- 한계/비용: `docs/04_limitations_and_costs.md`
- 학기 외 확장 결과: `docs/06_future_work_implementations.md`
- 연구 확장 계획: `docs/07_research_extension_plan.md`
- Combined IVI PoC: `data/reports/combined_ivi_chain_poc_20260518.md`
- PoC evidence index: `data/reports/poc_evidence_index.md`
- Stage 3 결과: `data/reports/stage3_ensemble.md/json` (local-only full n=47)
- Bootstrap CI: `data/reports/bootstrap_ci.md`
- McNemar test: `data/reports/mcnemar_test.md/json`
- Codex multi-model: `data/reports/codex_multimodel_agreement.md/json`
- RAG ablation: `data/reports/rag_ablation.md/json`
- Native sample: `data/reports/native_r3c2_20260514.md`
