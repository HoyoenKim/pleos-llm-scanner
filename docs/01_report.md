...
- 산출: `data/reports/codex_multimodel_agreement.{md,json}`
...
| Label | 의미 |
|---|---|
| L1 | Native 코드 분석 불가 (Java-only 정적 분석 한계) — **학기 외 C 작업으로 부분 해소**: radare2 + `src/native_analyze.py` 로 R3.c.2 native sample n=4 정적 분석 (추가 vuln 0건). dynamic 행동 (Go runtime / JNI argument flow) 은 여전히 미관찰 — Frida hook (학기 외 D, script ready) 후속 |
...
> - **2026-05-11 14주차 v1.1 — Future Work 5종 partial-implementation 통합** ([`docs/06_future_work_implementations.md`](06_future_work_implementations.md) 참조):
...
>   - **R3.c (radare2 native binary 분석)** — `tools/r2/radare2-6.1.4-w64/bin/radare2.exe` 통합 후 `libgojni.so` (15 MB Go binary, ai.pleos.sync.syslog) 정적 분석. Result: **transport secure (Go crypto/tls + x509 verification 활성), hardcoded endpoint 0건, 추가 vuln 0건**. R3.a corpus-level + R3.c sample-level 두 axis 완성. Static-only 한계 정직 명시.
...
>   - 산출: `data/reports/native_r3c2_20260514.md` + `data/reports/native_r3c2_raw.json`.
...
>   - **`docs/02_case_studies.md` Case 6~10 dynamic 추가** — 각 case 에 hook script + verdict transition table. Static-only vs Static+Dynamic 비교 표.
...
5. **사례 연구 10건**: `docs/02_case_studies.md` (정적 5건 + 학기 외 D Dynamic 5건)
6. **6개 시각화 차트**: `data/viz/01~06_*.png`
7. **학기 외 산출물**: `src/native_analyze.py` (C) + `src/rag/` 4 모듈 + `configs/prompts/stage1_detect_rag.md` (B) + `src/dynamic/state_machine.py` + `src/dynamic/hooks/` 5 Frida scripts (D) + `docs/06_future_work_implementations.md`
...
**R1.b 후속 — Bootstrap CI** (2026-05-06 도입, 2026-05-07 / 2026-05-14 갱신): `src/eval.py` 에 non-parametric percentile bootstrap (`--bootstrap N` 옵션) 도입. with-replacement 1000 회 resample (seed=42, deterministic).
...
**핵심 발견**: 표본 19 → 47 확장으로 **CI 폭이 약 40% 줄어든다** (Precision / FP rate 36.8%p → 21.3%p, F1 24.0%p → 13.1%p). Precision lower bound 57.9% → **70.2%** (+12.3%p) — 가설 25% FP rate 가 lower bound 훨씬 위에 위치. point 값은 R1.d.5 신규 FP 3건으로 83.8% → 80.9% 후퇴했으나 CI 는 계속 좁아짐 (point 변동 ≠ 통계적 보장 약화). RQ1 의 actionable insight: **n ≥ 40 corpus 부터는 보고서의 정량 주장이 통계적으로 더 신뢰 가능**. 산출: [`data/reports/bootstrap_ci.{md,json}`](../data/reports/bootstrap_ci.md).
...
**해석**: 한계 L1 (Java-only) 의 corpus-level 인덱스 = **PleOS 10.1%, 전체 10.6% APK 가 본 pipeline 의 blind spot**. 더 중요한 점은 차량 제어 / 맵 navigation / 음성 비서 같이 **보안 가치가 큰 컴포넌트가 native 비중 top** — 단순히 10% 가 아니라 "위험도 가중 10%" 로 해석해야. **RQ3.c (native-bound vuln 정량) 는 학기 외 C 작업으로 첫 sample-level 측정 완료** — radare2 6.1.4 + `src/native_analyze.py` 로 native sample n=4 정적 분석, 추가 vuln 0건 (§ 3.6.9 참조). 산출: [`data/reports/native_lib_inventory.{md,json}`](../data/reports/native_lib_inventory.md).
...
§ 3.6.3 의 corpus-level 인덱스 (10.6% APK 가 native lib 보유) 의 sample-level 첫 정량. 학기 외 C 작업으로 radare2 6.1.4 통합 + `src/native_analyze.py` (Windows / Git Bash binutils 부재 우회용 Python regex string 추출기) 작성.
...
- **사례 연구 5건 추가** — [`02_case_studies.md`](02_case_studies.md) Case 6~10 에 각 hook 의 verdict transition table + static-only vs static+dynamic 비교.
...
|---|---|---|
| ① jadx + 수동 분석가 | self GT 가 manual baseline (분석가 시간 6~7h) | 본 파이프라인 ~2h (3배 단축). 단, 정확한 stopwatch 측정은 Future Work ([`05_future_work.md`](05_future_work.md) F12) |
...
본 표는 `src/aaos_map.py` 의 n=19 시점 자동 산출 결과다. 본 corpus에서는 AAOS §3.7 / 4.2 / 5.1 만 커버하며, §4.1 Lockscreen, §6 OTA / Verified Boot 등은 미해당 → Future Work. **R1.d.2~d.5 신규 28 finding** (usb / statementservice / account / appmarket / ambientai / maps) 도 동일 3 섹션에 매핑된다 — usb-1/3 (§4.2), ss-1 (§3.7), acc-1/3/5 + am-1 + amb-3 (§5.1/§3.7), acc-2/4 + am-3 + amb-1/2 (§4.2). 전체 n=47 의 AAOS 매핑 표 재생성은 `src/aaos_map.py --labels combined_labels.json` 으로 가능 (학기 외 후속).
...
|---|---|---|---|
| **L1** | 네이티브 코드 분석 불가 | § 7.4 Tooling boundary | **부분 해소** — 학기 외 C 작업: radare2 + `src/native_analyze.py` 로 R3.c.2 sample n=4 정적 분석 (추가 vuln 0건, § 3.6.9). dynamic 행동은 Frida hook (학기 외 D, script ready) 후속 |
...
- ✅ Dynamic 검증 연계 — 학기 외 D 작업으로 deterministic state machine + 5 Frida hook script + 사례 연구 5건 추가 (§ 3.6.10, [`02_case_studies.md`](02_case_studies.md) Case 6~10).
