...
현재 연구 본선은 `docs/01_final_report.md` v1.4 supplement 기준으로 완료되어 있다. 2026-05-14까지의 정적 분석/RQ 측정 결과가 canonical이고, 2026-05-15~18의 runtime PoC/Binder evidence는 별도 runtime validation/FN expansion track으로 분리한다.
...
| `docs/01_final_report.md` | v1.4 supplement 통합 보고서. `n=47` 정적 평가와 Future Work A~E 완료 결과의 근거 저장소 |
| `docs/02_final_brief.md` | 처음 보는 평가자용 압축 최종본. 2026-05-18 기준 RQ5 runtime PoC 요약 포함 |
| `docs/04_methodology_stage2.md` | Stage 2 contextual verification 방법론 설명 |
| `data/reports/runtime_local/poc_evidence_index.md` | local-only runtime PoC evidence level index. 공개 전 redaction review 필요 |

## 3. 진행 이력 요약

| 날짜 | 큰 변화 |
|---|---|
| 2026-04-29 | repo scaffold, `configs/`, `scripts/apk/pull_apks.sh`, `scripts/apk/decompile.sh`, `src/evaluation/eval.py`, `src/evaluation/ablation.py`, self GT 시작 |
...
| `data/_local/apks/_external/` | NewPipe/InsecureBankv2 APK | local-only input corpus |
| `data/_local/runtime_poc_harness/` | 2026-05-18 PoC harness build output | repo 내부 ignored harness와 중복 가능. 정리 시 source만 남기고 build artifacts는 archive/local-only |
...
| `data/_local/apks/`, `data/_local/decompiled/` | ignored, very large | local-only. 공개/commit 금지 |
| `data/_local/native_extracts/`, `data/_local/rag_db/`, `data/_local/tools/` | ignored generated/binary | local-only. summary만 docs/reports에 기록 |
| `data/_local/poc_evidence/`, `data/_local/runtime_videos/` | runtime raw evidence/videos | local-only. redacted summary만 commit 후보 |
...
| Group | Examples | 이유 |
|---|---|---|
| docs/status sync | `docs/09_research_extension_plan.md`, `docs/02_final_brief.md`, `docs/04_methodology_stage2.md`, `docs/10_project_inventory.md` | reader-facing 또는 방법론/정리 문서 |
...
| Group | Examples | 이유 |
|---|---|---|
| APK/decompiled source | `data/_local/apks/`, `data/_local/decompiled/` | contractor/IP 보호 |
| raw PoC evidence | `data/_local/poc_evidence/`, `data/reports/runtime_local/_raw/` | screenshots/logcat/window dump 포함 |
| videos/previews | `data/_local/runtime_videos/`, `data/reports/runtime_local/prompt_leak_attack_poc/*.mp4`, preview PNGs | 발표 산출물 또는 민감 context 가능 |
| generated tools/db | `data/_local/tools/`, `data/_local/rag_db/`, `data/_local/native_extracts/deep_dive/` | binary/generated |
| build output | `data/_local/runtime_poc_harness/classes*`, `*.apk`, `*.dex`, `debug.keystore` | 재생성 가능 산출물 |
...
| 상위 `data/_local/runtime_poc_harness` build artifacts | source와 manifest만 남기고 build output archive |
...
3. **public/sanitized 후보 선별**: `data/reports/aggregate/`, `data/reports/external/`, `data/reports/public/` 중 발표/보고서가 참조하는 summary만 review 후 exception 추가 또는 docs로 이동.
4. **PoC evidence index 중심화**: raw evidence는 `data/_local/poc_evidence/`에 두고, PPT/보고서가 인용하는 경로는 `poc_evidence_index.md` 하나에서 관리.
...
- `data/reports/runtime_local/poc_evidence_index.md`와 runtime summary reports에 secret/prompt 원문이 없는지 `rg`로 재검사.
- `scripts/runtime_poc/research_runtime_poc_harness.py`에 local absolute path, private filename, credential literal이 없는지 확인.
