...
본 학기 (2026-1) 안 완료한 RQ 측정값과 정직한 한계를 [`01_final_report.md`](01_final_report.md) § 7 / § 8 에 정리했다. 본 문서는 그 한계를 해소하는 **학기 외부 후속 작업 17 종** 을 4 카테고리로 정리한다.

> **학기 외 진행 현황 (2026-05-14)**: 본 17종 중 우선순위 상위 항목이 학기 외 A~E 작업으로 진행 완료되었다. 구현 도달도 + 설계 명세는 [`08_future_work_implementations.md`](08_future_work_implementations.md) 가 canonical.
...
3. `bash scripts/apk/decompile.sh data/_local/apks/<sample>.apk` (1~2분)
4. Claude Code 세션 진입 → keyword grep + Stage 1 분석 (1~2분)
5. 결과 JSON / md 출력 (즉시)
6. `python src/evaluation/eval.py --bootstrap 1000` 으로 측정값 표시
...
- **외부 corpus reports**: `data/reports/external/{UnCrackable-Level1/2/3, r2pay-v1.0, InsecureBankv2}_*.{md,json}`
- **결정론 측정 스크립트**: `src/evaluation/{eval, ablation}.py`, `src/mapping/{aaos_map, tara_generate}.py`, `src/deobf/entropy.py`, `src/viz/plot_metrics.py`, `scripts/research/research_r{1a, 1c, 2a, 2b, 3a}_*.py`, `scripts/research/research_random_baseline.py`
