...
_작성: 2026-04-30. 보고서 7장 (Limitations) + 8장 (Cost Analysis) 의 raw 분석 노트. 2026-05-14 학기 외 A~E 반영 갱신._

본 문서는 본 파이프라인의 **구조적 한계**와 그 **근본 원인**, 그리고 운영 시점에서의 **시간/비용 트레이드오프** 를 정리한다. 보고서 ([`01_final_report.md`](01_final_report.md) § 7 / § 5.5) / 발표 / 후속 연구의 input.

> **⚠️ 갱신 안내 (2026-05-14)**: 본 노트의 측정값은 n=18/n=19 시점 기준이다. 학기 외 A~E 작업으로 (1) **L1 (native) 부분 해소** — radare2 + `src/native/native_analyze.py` 로 R3.c.2 native sample n=4 정적 분석 (추가 vuln 0건), (2) **L2 (표본) n=19 → n=47 해소** — McNemar p_exact=0.0215 통계적 유의 도달, (3) **L3 보충 실험 완료** — Codex 3-model cross-read는 Claude baseline을 능가하지 못함, (4) **L6 신규** — ProGuard 활성 APK 의 priority class yield 감소. 최종 한계 표 + 상태는 [`01_final_report.md`](01_final_report.md) § 7.0 / Notation L1~L6 참조. 운영 비용 (§ 2) 수치는 변동 없음.
...
|---|---|---|---|---|
| L1 | **네이티브 코드 분석 불가** ← 2026-05-14 **부분 해소** (학기 외 C) | jadx = Java/Kotlin 전용 디컴파일러 | Ghidra/IDA 등 binary 도구 미통합 | UnCrackable-Level2 + r2pay-v1.0 in-scope 0건. **학기 외 C: radare2 + `src/native/native_analyze.py` 로 R3.c.2 native sample n=4 정적 분석 — 추가 vuln 0건** (`data/reports/native_local/native_r3c2_20260514.md`). dynamic 행동은 여전히 미관찰 (Frida hook 학기 외 D, script ready) |
...
| L3 | **멀티 모델 앙상블 미구현 → Codex 보충 실험 완료** | Claude Code 단일 모델 세션 | 외부 유료 API 미사용 정책 → true multi-vendor API 비교는 불가 | 3차 검증은 멀티 프롬프트(동일 모델 + 시각 3종)로 전환. 학기 외 E에서 Codex 3-model cross-read 수행: Codex 2/3 F1 86.6%, Claude baseline F1 98.7%. 결론은 multi-model 자체보다 prompt calibration + evidence packaging 중요 |
...
- 본 파이프라인의 entry point가 `scripts/apk/decompile.sh data/_local/apks/<X>.apk` → jadx 호출 → java sources만 출력. ELF는 무시됨.
...
#### 해소 결과 (2026-04-30)
- ucl1-1/2/3 + ucl3-1 4건에 attacker/defender/domain_expert 평가 추가 → `data/reports/local/stage3_ensemble.json` 갱신
...
| **이식성** | 60~70% OS-독립 ([`01_final_report.md`](01_final_report.md) § 6) | swap 필요: 디컴파일러 / 키워드 룰셋 / 매핑 yaml |
...
- `docs/01_final_report.md` 7장 (Limitations) + 8장 (Cost Analysis) 의 raw 노트
...
- `docs/01_final_report.md (§ 6)` (이식성 trade-off)
...
| 2026-05-14 | 학기 외 A~E 반영 — L1 부분 해소 (radare2 R3.c.2 native n=4), L2 해소 (n=47 + McNemar 유의), L3 Codex multi-model 보충 실험 완료, L6 신규 (ProGuard yield). § 1 한계 표 갱신 + 상단 갱신 안내 추가. 운영 비용 (§ 2) 수치는 변동 없음. |
