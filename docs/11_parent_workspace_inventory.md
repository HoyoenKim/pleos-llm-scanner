...
| `pleos-llm-scanner/data/_local/apks/_mastg/owasp-mastg/.git` | Local OWASP MASTG input clone under ignored data | Local-only reference corpus; do not push as part of this repo |
...
| `data/_local/apks/_external/` | External APK inputs (`NewPipe.apk`, `InsecureBankv2.apk`) | Archived on 2026-05-19 because matching copies already exist under `pleos-llm-scanner/data/_local/apks/_external/` |
| `data/_local/runtime_poc_harness/` | Generated runtime PoC harness source/build outputs | Treat as local-only generated evidence; do not push APK/dex/keystore/class outputs |
...
| External APK inputs | `data/_local/apks/_external/NewPipe.apk`, `data/_local/apks/_external/InsecureBankv2.apk` |
| Runtime harness outputs | `data/_local/runtime_poc_harness/*.apk`, `*.idsig`, `debug.keystore`, `classes.jar`, `dex/classes.dex`, compiled classes |
...
4. If the harness source is needed later, regenerate it from the tracked repo scripts instead of treating parent `data/_local/runtime_poc_harness/` as the canonical source.
...
| `data/_local/runtime_poc_harness/` | Generated PoC harness APK/dex/keystore/class outputs |
| `data/_local/apks/` | Duplicate parent-level external APK cache; SHA256 matched repo-local ignored copies |
