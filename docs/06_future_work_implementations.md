...
- `scripts/research_r2c_codex_multimodel.py`
- `data/reports/codex_multimodel/sample19_agreement.{md,json}`
...
| **C. L1 해소 — native 분석** | ✅ **완료** (static portion) | **100%** | radare2 + `src/native_analyze.py` + R3.c.2 native sample n=4 (추가 vuln 0건). Ghidra Go plugin function-naming 은 deferred sub-task. § 1 |
...
Week 4 ────── 통합 보고서 v1.2 + 18-슬라이드 final PPT
```
...
| 항목 | Done 기준 |
|---|---|
| **A R1.d.5** | n≥40 / Stage 3 ≥2/3 측정 갱신 / McNemar p값 (유의/비유의 모두 정직 기록) / `data/reports/bootstrap_ci.md` n≥40 + `mcnemar_test.md` 갱신 / 보고서 v1.1 → v1.2 |
...
| **D Dynamic** | frida-server 동작 / 5 strong-TP finding (vc-5/6, ssl-2/5, lmp-1) Frida hook script + 실제 emulator log / `src/dynamic/state_machine.py` (LangGraph state-only) / Stage 3 cross-correlate 결과 5건 / `docs/02_case_studies.md` dynamic 사례 5건 추가 / 보고서 v1.2 § Dynamic verify |
...
- Intrinsic ablation (n=47): **NN verdict propagation 85.1% / AAOS category alignment 85.1% / MASVS area match 23.4%**. 산출: `data/reports/rag_ablation.{md,json}`.
- End-to-end prompted-LLM ablation (with/without RAG) 은 별도 work-load 후속.

### C. L1 native 분석 ✅ (static portion)

- `src/native_analyze.py` (Python regex string 추출 — Windows binutils 부재 우회).
...
- 핵심 발견: native lib = Java BuildConfig 의 consumer 패턴. 산출: `data/reports/native_r3c2_20260514.md`.
...
- `docs/02_case_studies.md` Case 6~10 dynamic 추가 (verdict transition table + static-only vs static+dynamic 비교).
...
- 산출: `configs/prompts/codex_multimodel_cross_read.md` + `scripts/research_r2c_codex_multimodel.py` + `data/reports/codex_multimodel/*` + `data/reports/codex_multimodel_agreement.{md,json}`.

---

## 1. radare2 native binary 분석 — 학기 외 C 작업 완료

본 학기 환경 정비 (`tools/r2/radare2-6.1.4-w64/bin/radare2.exe`) 후 첫 native binary 분석 진행. 상세 산출은 [`data/reports/native_libgojni_20260511.md`](../data/reports/native_libgojni_20260511.md).
...
> ✅ **학기 외 D 완료 (2026-05-14)** — 본 절은 설계 명세다. **구현 결과 (deterministic `src/dynamic/state_machine.py` — LLM node 제거 + 5 Frida hook script + 사례 연구 5건) 는 § 0.2 D + 보고서 § 3.6.10 + [`02_case_studies.md`](02_case_studies.md) Case 6~10 참조.** AVD runtime capture 만 환경 의존 후속.
...
| C. L1 native 분석 | 35% (libgojni n=1) | ✅ **100%** (static) | `src/native_analyze.py` + R3.c.2 native sample n=4 / 추가 vuln 0건 |
